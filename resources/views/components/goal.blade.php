...
<div class="relative col-span-2 shadow-sm p-5 rounded-lg min-h-72 bg-purple-400  flex justify-between flex-col   ">
    <div class="flex justify-between items-start z-20">   
      <div class="">
        <h4 class="font-semibold text-sm text-slate-100">{{ $data->target_value }} :</h4>
        <h4 class="font-semibold text-lg text-slate-100">{{ $data->current_value }}</h4>

      </div>
        
        <span class="relative flex h-12 w-12 group cursor-pointer ">
          <div class="group-hover:block hidden bg-white text-slate-500 rounded absolute -translate-x-5 -top-4 left-0  z-20 text-sm px-5">{{ $data->timeline }}</div>
          <span class="animate-ping absolute inline-flex bg-gradient-to-b from-purple-400  h-full w-full rounded-full opacity-75
          {{ $data->current_persen < 35 ? 'to-red-400' : ($data->current_persen < 70 ? 'to-yellow-300' : 'to-green-400') }}
          "></span>
          <span class="relative rounded-full h-12 w-12 border-2 bg-slate-50 flex justify-center items-center 
          {{ $data->current_persen < 35 ? 'border-red-300' : ($data->current_persen < 70 ? 'border-yellow-300' : 'border-green-400') }}
          ">
           <p class="text-slate-600 text-sm ">{{ $data->current_persen }}%</p>
          </span>
        </span>

    </div>
    <div class="flex justify-between items-center z-20">   
        <h4 class="font-bold text-2xl text-slate-100  00">{{ $data->title }}</h4>

        <button id="doubleDropdownButton{{ $data->id }}" data-dropdown-toggle="doubleDropdown{{ $data->id }}" data-dropdown-placement="right-start" type="button" class="bg-white opacity-75 w-10 h-10  rounded-full hover:scale-110">
          <span class="material-symbols-outlined -translate-x-0 text-lg text-black  ">more_vert</span>
        </button>  
    </div>

    <div class="w-2/3 h-full absolute bg-gradient-to-r from-purple-400 to-transparent z-10 right-0 top-0"></div>

    <img src="{{ asset('storage/' . $data->image)  }}" class="w-2/3 absolute right-0 top-0 h-full opacity-50 z-0 object-cover object-right" alt="" >


</div>

{{-- dropdown --}}
<div id="doubleDropdown{{ $data->id }}" class="z-30 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 cursor-pointer">
  <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="doubleDropdownButton">
    <li>
      <a data-modal-target="edit{{ $data->id }}" data-modal-toggle="edit{{ $data->id }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
    ...
    <li>
      <a data-modal-target="delete{{ $data->id }}" data-modal-toggle="delete{{ $data->id }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
        <span class="material-symbols-outlined text-sm me-2">
          delete
          </span>
        Delete</a>
...
<x-modal idmodal="edit{{ $data->id }}">
  <div class="relative w-[50rem] bg-white rounded-lg shadow dark:bg-gray-700 -translate-y-4 pb-4">
      ...
      <form method="post" action="{{ url('goals/'. $data->id ) }}" class="flex md:flex-row flex-col gap-4 p-4 md:p-5 justify-between" enctype="multipart/form-data">
        @method("put") 
        @csrf
          <div class="flex  items-center justify-center md:w-1/2">
              <label for="dropzone-file{{ $data->id }}"
                  ...
                  <img src="{{ asset('storage/'. $data->image) }}" class="absolute w-full h-full object-contain" alt="">
                  <div class="flex flex-col z-20 items-center justify-center pt-5 pb-6 bg-[rgba(255,255,255,.8)] rounded-lg" >
          ...
          <div class=" flex flex-col md:w-1/2 gap-3 px-5">                
              ...
              <div class="w-1/2">
                <label for="status" class="block mb-1 text-sm font-medium text-slate-700 dark:text-white">Status :</label>
                <select name="status" id="status" class="block w-full bg-gray-50 border border-gray-300 text-slate-700 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option value="notstart" {{ $data->status == "notstart" ? "selected" : "" }}>not started</option>
                    <option value="inprogres" {{ $data->status == "inprogres" ? "selected" : "" }}>in progres</option>
                    <option value="completed" {{ $data->status == "completed" ? "selected" : "" }}>finsh</option>
                </select>
            </div>
